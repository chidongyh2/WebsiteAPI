...
namespace GHM.Warehouse.Infrastructure.Services
{
    public class ProductCategoryService : IProductCategoryService
    {
        ...
        public ProductCategoryService(IProductCategoryRepository productCategoryRepository, IProductCategoryTranslationRepository productCategoryTranslationRepository,
            IResourceService<SharedResource> sharedResourceService, IResourceService<GhmProductResource> productResourceService,
            IProductCategoriesAttributeRepository productCategoriesAttributeRepository, IProductAttributeRepository productAttributeRepository,
        ...
        public async Task<ActionResultResponse<int>> Insert(string tenantId, string creatorId, string creatorFullName, ProductCategoryMeta productCategoryMeta)
        {
            ...
            if (productCategoryMeta.ProductCategoryAttributes != null && productCategoryMeta.ProductCategoryAttributes.Any())
            {
                ...
                {
                    var isExistsAttribute = await _productAttributeRepository.CheckExists(item.AttributeId, tenantId);
                    if (!isExistsAttribute)
                        return new ActionResultResponse<int>(-1, _productResourceService.GetString("ProductAttribute not exists"));
        ...
        public async Task<ActionResultResponse> Update(string tenantId, string lastUpdateUserId, string lastUpdateFullName, int productCategoryId, ProductCategoryMeta productCategoryMeta)
        {
            ...
            async Task<ActionResultResponse> UpdateProductCategoryAttribute()
            {
                ...
                foreach (var item in productCategoryMeta.ProductCategoryAttributes)
                {
                    var isExistsProductAttribute = await _productAttributeRepository.CheckExists(item.AttributeId, tenantId);
